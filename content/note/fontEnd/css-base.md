...
1. 开启 BFC 的元素**不会被浮动元素所覆盖**
2. 开启 BFC 的元素子元素和父元素**外边距不会重叠**
3. 开启 BFC 的元素可以包含浮动的子元素
4. 父子元素和兄弟元素边距重叠，重叠原则取最大值。
5. 空元素的边距重叠是取 margin 与 padding 的最大值。
...
4. 找到要设置图标的元素通过 before 或 after 选中
5. 在 content 中设置字体的编码
6. 设置字体的样式
