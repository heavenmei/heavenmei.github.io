...
var checksum = (content2) => {
  return crypto.createHash("sha256").update(content2).digest("hex");
};
...
var metadata = async (resourcePath, source, pathname) => {
  const content2 = readFileSync(path.join(process.cwd(), "/public/avatar.jpg"));
  console.log("staticImages ===", "./" + source);
  const image = await sharp(content2);
  const { width, height } = await image.metadata();
  if (!width || !height) {
    return null;
  }
  const src = resourcePath + "/" + pathname;
  const sha256 = checksum(content2);
...
var processImage = async (options, file, node) => {
  ...
  console.log("staticImages ===", options, pathname, source);
...
// contentlayer.config.js
var content = readFileSync2(path2.join(process.cwd(), "/public/avatar.jpg"));
console.log("sdhjsdhjshd====", content);
...
function createTagCount(allBlogs) {
  ...
  writeFileSync2("public/tag-data.json", JSON.stringify(tagCount));
}
  ...
  mdx: {
    ...
    rehypePlugins: [
      [
        ...
        {
          publicDir: path2.join(process.cwd(), "public/posts"),
          sourceRoot: "content"
        }
      ],
      // preProcess,
...
//# sourceMappingURL=compiled-contentlayer-config-7PQR5T6M.mjs.map
