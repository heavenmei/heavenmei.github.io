...
// lib/placeholder.ts
var createPlaceholder = async (image2) => {
  ...
  return image2.resize(placeholderImgWidth, placeholderImgHeight).png({
    quality: 75
...
var metadata = async (resourcePath, source, pathname) => {
  ...
  const image2 = await sharp(content);
  const { width, height } = await image2.metadata();
  if (!width || !height) {
    return null;
  }
  ...
  const blurDataURL = await placeholder_default(image2);
  return {
    sha256,
    props: {
      width,
...
//# sourceMappingURL=compiled-contentlayer-config-S2MYXDGB.mjs.map
